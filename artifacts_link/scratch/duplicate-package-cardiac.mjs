...
async function run() {
  ...
  profilesSnap.forEach(doc => {
    const data = doc.data();
    if (!data.isDeleted && data.name && (data.name.toLowerCase().includes("men") || data.name.toLowerCase().includes("women"))) {
      sourcePkg = data;
  ...
  if (!sourcePkg) {
    console.error("Error: Could not find any active checkup package to copy from.");
    return;
  }
  ...
  const duplicatePkg = {
    ...
    price: 2990,
    discountPrice: null // Clear any discount pricing so it uses the set price
  };

  console.log("Adding duplicate package with name: " + duplicatePkg.name + " (Price: ₹" + duplicatePkg.price + ")");
